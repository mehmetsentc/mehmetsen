#!/usr/bin/env node
/**
 * Sync Antalya Eczacı Odası on-duty pharmacies into Firestore `dutyPharmacies`.
 *
 * Usage:
 *   npm run sync-antalya-nobetci-eczane                 # direct Admin SDK (default)
 *   npm run sync-antalya-nobetci-eczane -- --http       # POST /api/cron/antalya-nobetci-eczane
 *
 * Requires Firebase Admin credentials in .env.local for direct mode, or
 * CRON_SECRET / EVENTS_SYNC_SECRET for HTTP mode.
 *
 * Vercel cron: vercel.json → /api/cron/antalya-nobetci-eczane at 09:30 Europe/Istanbul.
 */
...
function runDirectSync() {
  ...
  const script = join(root, 'scripts', 'sync-antalya-nobetci-eczane-direct.ts')
  if (!existsSync(tsxBin)) {
    console.error('Missing tsx. Run: npm install')
    process.exit(1)
  }

  console.log('Running Antalya nöbetçi eczane sync (Firebase Admin SDK)…\n')
...
async function runHttpSync() {
  ...
  const url = `${baseUrl}/api/cron/antalya-nobetci-eczane`
