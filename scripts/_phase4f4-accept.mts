/**
 * Phase 4F.4 — controlled continuous paid auto-draft pilot helpers.
 * Modes: baseline | costs | observe | fresh | event | jobs-after | firewall | sources
 * Never prints secrets. No provider spend from this script.
 */
...
const outPath = process.argv[4] || `tmp-phase4f4-${mode}.json`
...
function t4f4(): string {
  return (
    (existsSync('tmp-phase4f4-T4F4.txt')
      ? readFileSync('tmp-phase4f4-T4F4.txt', 'utf8').trim()
...
async function main() {
  ...
  const T4F4 = t4f4()
  const out: Record<string, unknown> = { mode, at: new Date().toISOString(), T4F4, HIST_PROTECT }

  if (mode === 'baseline' || mode === 'observe' || mode === 'costs') {
    ...
    out.ledger_after_T4F4 = (
      await sql`
      SELECT coalesce(sum(actual_cost_usd),0)::float AS cost, count(*)::int AS n
      FROM crawler_ai_cost_ledger
      WHERE timestamp > ${T4F4}::timestamptz`
    )[0]
    out.jobs_after_T4F4 = await sql`
      ...
      WHERE created_at > ${T4F4}::timestamptz
      ORDER BY created_at`
    ...
    out.hist_jobs_after = await sql`
      ...
      WHERE cluster_id = ANY(${HIST_PROTECT})
        AND created_at > ${T4F4}::timestamptz
      GROUP BY 1`
    out.auto_pubs_after = (
      await sql`
      SELECT count(*)::int AS c FROM crawler_ai_jobs j
      JOIN news n ON n.id = j.editorial_news_id
      WHERE j.created_at > ${T4F4}::timestamptz
        AND n.published_at IS NOT NULL
        AND n.published_at > ${T4F4}::timestamptz`
    )[0]
  ...
  if (mode === 'fresh') {
    out.fresh = await sql`
      ...
      WHERE c.published_news_id IS NULL
        AND c.created_at >= ${T4F4}::timestamptz
      ...
      SELECT id, status::text AS status, eligibility_status, blocked_reason,
        ...
        CASE WHEN draft_snapshot IS NOT NULL THEN
          coalesce((draft_snapshot->>'bodyWordCount')::int,
                   (draft_snapshot->>'body_word_count')::int, 0)
