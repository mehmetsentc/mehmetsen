...
# ============================================================
# NaHaber v1.2 Build 4 — Build & Upload
# Düzeltmeler:
#   - Apple Sign In fix (iOS 26 / Guideline 2.1a)
#   - UGC moderation: flag/block/EULA (Guideline 1.2)
#   - entitlements: com.apple.developer.applesignin = Default
# ============================================================

PROJECT_DIR="/Users/user/nahaber"
IOS_DIR="$PROJECT_DIR/ios/App"
ARCHIVE_PATH="/Users/user/Downloads/NaHaber_1.2_b4.xcarchive"
EXPORT_DIR="/Users/user/Downloads/NaHaber_IPA_b4"
...
BUILD_KEYCHAIN="/tmp/nahaber_build4.keychain-db"
BUILD_KP="NaHaber_Build_2026"

KEY_P8="$PROJECT_DIR/AuthKey_88PX7Q6W29.p8"
KEY_ID="88PX7Q6W29"
ISSUER_ID="0b4b2878-8080-476e-aafe-0bd515dce30c"
LOG="/Users/user/Downloads/build_upload_b4.log"

echo "============================================================" | tee "$LOG"
echo "NaHaber v1.2 Build 4 — Build & Upload" | tee -a "$LOG"
...
echo "=== Entitlements kontrolü ===" | tee -a "$LOG"
ENTITLEMENTS="$IOS_DIR/App/App.entitlements"
if [ ! -f "$ENTITLEMENTS" ]; then
  echo "HATA: App.entitlements bulunamadı: $ENTITLEMENTS" | tee -a "$LOG"
  exit 1
fi
echo "✅ Entitlements mevcut" | tee -a "$LOG"
echo "" | tee -a "$LOG"

# --- Git lock temizle ve commit_ugc.sh çalıştır (eğer commit yapılmadıysa) ---
echo "=== Git durumu ===" | tee -a "$LOG"
cd "$PROJECT_DIR"
rm -f .git/HEAD.lock .git/index.lock 2>/dev/null || true
git status --short | tee -a "$LOG"
echo "" | tee -a "$LOG"

# Uncommitted değişiklik varsa commit et
if [ -n "$(git status --porcelain)" ]; then
  echo "=== Bekleyen değişiklikler commit ediliyor ===" | tee -a "$LOG"
  bash "$PROJECT_DIR/commit_ugc.sh" 2>&1 | tee -a "$LOG" || true
fi
...
# --- xcodebuild archive ---
echo "=== Xcode Archive (v1.2 Build 4) ===" | tee -a "$LOG"
rm -rf "$ARCHIVE_PATH"
cd "$IOS_DIR"
xcodebuild archive \
...
if [ -z "$IPA_FILE" ]; then
  echo "HATA: IPA dosyası bulunamadı! Export başarısız olmuş olabilir." | tee -a "$LOG"
  exit 1
fi
...
echo "" | tee -a "$LOG"
echo "============================================================" | tee -a "$LOG"
echo "✅ TÜM İŞLEMLER TAMAMLANDI — v1.2 Build 4" | tee -a "$LOG"
echo "Log: $LOG" | tee -a "$LOG"
echo "============================================================" | tee -a "$LOG"
echo ""
echo "Şimdi App Store Connect'te 'Resubmit to App Review' tıkla:"
