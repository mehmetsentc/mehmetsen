...
describe('P6 event slug derivation', () => {
  it('turkish-normalizes title into stable slug', async () => {
    const { deriveEventSlug, eventSlugCandidates } = await import('@/lib/seo/eventSlug')
    const slug = deriveEventSlug('Çanakkale Boğazı trafiği', null, 'cl_1')
    expect(slug).toBe('canakkale-bogazi-trafigi')
    expect(eventSlugCandidates(slug, 'cl_abc12345')[0]).toBe(slug)
    expect(eventSlugCandidates(slug, 'cl_abc12345').length).toBeGreaterThan(2)
  })

  it('prefers eventKey when present', async () => {
    const { deriveEventSlug } = await import('@/lib/seo/eventSlug')
    expect(deriveEventSlug('Başlık', 'EVT-KEY-1', 'cl_x')).toBe('evt-key-1')
  })
})
