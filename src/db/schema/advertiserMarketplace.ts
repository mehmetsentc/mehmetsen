...
export const adBookings = pgTable(
  'ad_bookings',
  {
    ...
    currency: varchar('currency', { length: 3 }).default('TRY').notNull(),
    pricingModelSnapshot: varchar('pricing_model_snapshot', { length: 32 }).notNull(),
    /** P10A immutable commercial snapshot (frozen at booking create / intent). */
    grossAmountMinor: bigint('gross_amount_minor', { mode: 'number' }),
    platformCommissionRateBps: integer('platform_commission_rate_bps'),
    platformCommissionMinor: bigint('platform_commission_minor', { mode: 'number' }),
    publisherGrossMinor: bigint('publisher_gross_minor', { mode: 'number' }),
    publisherNetMinor: bigint('publisher_net_minor', { mode: 'number' }),
    taxPlaceholderMinor: bigint('tax_placeholder_minor', { mode: 'number' }),
    invoiceStatus: varchar('invoice_status', { length: 32 }),
    taxProfileId: varchar('tax_profile_id', { length: 64 }),
    commercialSnapshotAt: timestamp('commercial_snapshot_at', { withTimezone: true }),
    commercialFrozen: boolean('commercial_frozen').default(false).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
