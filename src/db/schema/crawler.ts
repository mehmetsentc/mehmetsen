...
export const discoveredArticleUrls = pgTable(
  'discovered_article_urls',
  {
    ...
    logicalQueue: varchar('logical_queue', { length: 32 })
      ...
      .notNull(),
    discoveryLane: varchar('discovery_lane', { length: 24 }).default('CRAWLER'),
    discoveryLanes: jsonb('discovery_lanes').$type<string[]>().default(sql`'[]'::jsonb`),
    titleHint: text('title_hint'),
    guid: varchar('guid', { length: 500 }),
    discoveryPrimaryImageCandidate: text('discovery_primary_image_candidate'),
    rssDescription: text('rss_description'),
    feedMetadata: jsonb('feed_metadata').$type<Record<string, unknown>>(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
...
export const newsClusters = pgTable(
  'news_clusters',
  {
    ...
    primarySelectionScore: real('primary_selection_score'),
    primarySelectionReasons: jsonb('primary_selection_reasons').$type<string[]>(),
    publishedNewsId: varchar('published_news_id', { length: 64 }),
    futureAiUnit: varchar('future_ai_unit', { length: 24 }).default('PREPARED'),
    updateReviewStatus: varchar('update_review_status', { length: 24 }).default('NONE'),
  ...
  (t) => [
    ...
    index('news_clusters_seo_slug_idx').on(t.seoSlug),
...
export const rawArticles = pgTable(
  'raw_articles',
  {
    ...
    qualityGateReasons: jsonb('quality_gate_reasons').$type<string[]>(),
    rssSnippetUsedAsBody: smallint('rss_snippet_used_as_body').default(0).notNull(),
    clusterRole: varchar('cluster_role', { length: 24 }),
    discoveryPrimaryImageCandidate: text('discovery_primary_image_candidate'),
    primaryImageConfidence: real('primary_image_confidence'),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
...
export const clusterMemberships = pgTable(
  'cluster_memberships',
  {
    ...
    membershipRole: varchar('membership_role', { length: 24 }).default('SUPPORTING'),
    isIndependentSource: smallint('is_independent_source').default(1).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
