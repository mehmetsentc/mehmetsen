...
export const userContentImpressions = pgTable(
  'user_content_impressions',
  {
    ...
    // No FK to news.id — LEGACY_ALLOWED FS-only ids must persist for restart no-replay.
    ...
    /**
     * Qualified feed impression count (>=60% / >=750ms).
     * Article-open rows may insert with 0 so analytics stay unpolluted while still
     * marking durable consumed eligibility.
     */
  ...
  (t) => [
    ...
    index('user_content_impressions_user_last_seen_idx').on(t.userId, t.lastSeenAt),
    index('user_content_impressions_session_last_seen_idx').on(t.sessionId, t.lastSeenAt),
  ]
)
