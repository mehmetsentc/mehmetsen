...
export function resolveCountryFromText(text: string): WorldCountry | null {
  ...
  const TEXT_ALIASES: Array<[RegExp, string]> = [
    ...
    // İdlib → Suriye (olay yeri); "sam" yalnız başına çok zayıf, damascus/şam kullan
    [asciiWord('suriye|syria|damascus|sam|idlib|idlip|halep|aleppo'), 'suriye'],
    ...
    // Çizme = İtalya lakabı (futbol dilinde); "orta saha" ile karışmaz
    [asciiWord('italya|italy|roma|rome|milan|cizme'), 'italya'],
    [asciiWord('serie\\s*a'), 'italya'],
    [asciiWord('premier\\s*league'), 'birleşik krallık'],
    [asciiWord('la\\s*liga'), 'ispanya'],
    [asciiWord('bundesliga'), 'almanya'],
    [asciiWord('ligue\\s*1'), 'fransa'],
