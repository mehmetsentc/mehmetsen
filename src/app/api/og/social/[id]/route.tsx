...
async function fetchArticle(id: string): Promise<ArticleOGData | null> {
  ...
  try {
    const res = await fetch(`${FIREBASE_URL}/${id}?key=${apiKey}`, { next: { revalidate: 300 } })
...
// Node.js runtime: JPEG, PNG, GIF, WebP, AVIF hepsi desteklenir
const SUPPORTED_EXTS = /\.(jpe?g|png|gif|webp|avif)(\?|$)/i
const UNSUPPORTED_EXTS = /\.(svg|bmp|tiff?)(\?|$)/i

function isValidImageUrl(url: string | undefined): url is string {
  if (!url || !url.startsWith('http')) return false
  if (url.endsWith('/')) return false
  if (url.endsWith('-') || url.endsWith('_')) return false
  const lastSegment = url.split('/').pop() ?? ''
  if (lastSegment.length < 4) return false
  if (UNSUPPORTED_EXTS.test(url)) return false
  return true
}
      ...
      (
        <div style={{
          ...
          <div style={{
            ...
            {photo ? (
              ...
              <div style={{
                ...
                <div style={{ position: 'absolute', width: 480, height: 480, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.06)', bottom: -160, right: -80, display: 'flex' }} />
                <div style={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.08)', bottom: -80, right: 40, display: 'flex' }} />
                <div style={{ position: 'absolute', width: 180, height: 180, borderRadius: '50%', backgroundColor: 'rgba(59,130,246,0.12)', top: 60, left: -40, display: 'flex' }} />
              </div>
            )}
            <div style={{
              position: 'absolute', top: 24, right: 24,
              backgroundColor: '#0d1f44', borderRadius: 10,
              display: 'flex', alignItems: 'center',
              padding: '10px 22px 10px 10px', gap: 10,
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, background: 'linear-gradient(to top, rgba(0,0,0,0.45) 0%, transparent 100%)', display: 'flex' }} />
          </div>
          ...
          <div style={{
            ...
            <div style={{ color: 'white', fontSize: titleFontSize, fontWeight: 900, lineHeight: 1.28, display: 'flex', flexDirection: 'column' }}>{title}</div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ position: 'absolute', bottom: -60, right: -60, width: 280, height: 280, borderRadius: '50%', background: 'radial-gradient(circle, rgba(6,182,212,0.25) 0%, rgba(14,116,144,0.12) 50%, transparent 70%)', display: 'flex' }} />
            <div style={{ position: 'absolute', bottom: 10, right: 80, width: 180, height: 80, borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(6,182,212,0.18) 0%, transparent 70%)', display: 'flex' }} />
            <div style={{ display: 'flex', flex: 1, alignItems: 'center' }}>
              {spot ? (
                <span style={{ color: 'rgba(255,255,255,0.95)', fontSize: 28, fontWeight: 500, lineHeight: 1.5, display: 'flex' }}>{spot}</span>
              ) : (
                <span style={{ color: 'rgba(255,255,255,0.55)', fontSize: 26, fontStyle: 'italic', display: 'flex' }}>nahaber.com'da haberin devamını okuyun</span>
              )}
            </div>
            <div style={{ display: 'flex' }}>
              <span style={{ color: 'rgba(255,255,255,0.60)', fontSize: 15, fontWeight: 600, letterSpacing: 3, display: 'flex' }}>DAHA FAZLASI iCiN: WWW.NAHABER.COM</span>
            </div>
          </div>
        </div>
      ),
