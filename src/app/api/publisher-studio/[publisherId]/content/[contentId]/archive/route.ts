...
export async function POST(request: Request, context: RouteContext) {
  ...
  const auth = await withContentAuth(request, publisherId, 'content:archive')
  if ('error' in auth && auth.error) return auth.error
  try {
    const item = await publisherContentService.archive(publisherId, contentId, auth.auth!.user.uid)
