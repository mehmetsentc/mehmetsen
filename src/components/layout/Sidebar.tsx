...
import { Search, LogOut, CalendarDays, Clapperboard, Settings, Shield } from 'lucide-react'
    ...
    <>
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <button
              type="submit"
              aria-label="Ara"
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-orange-500 text-white hover:bg-orange-600"
            >
        ...
        <nav className="flex-1 overflow-y-auto">
          ...
          >
            {isFeed && !activeCategory && (
              <span className="absolute left-0 top-0 h-full w-[3px] rounded-r-full bg-orange-500" />
            )}
            Tümü
          </Link>
        ...
        <div className="shrink-0 space-y-1 border-t border-[rgb(var(--color-border))] p-3">
          ...
          {hydrated && !user && (
            <Link
              href={ROUTES.LOGIN}
              onClick={onMobileClose}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-orange-600"
            >
              Giriş Yap →
            </Link>
          )}
