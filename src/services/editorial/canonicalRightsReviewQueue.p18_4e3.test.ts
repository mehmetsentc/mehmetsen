...
describe('P18.4E.3 rights review UX helpers', () => {
  ...
  it('CMS page has batch filter, risk sort, confirms, and defers cohort publish', () => {
    ...
    expect(page).toContain('Benzerlik skoru kanıttır')
    expect(page).toContain('Yüksek benzerlik uyarısı')
    expect(page).toContain('DO_NOT_PUBLISH')
    expect(page).toContain('deferPublish')
    expect(page).toContain('Kohort yayını ertelendi')
