...
describe('DiscoveryRail Reader authority when enabled', () => {
  ...
  it('Haberi Oku remains a button (no nested /haber Link)', () => {
    ...
    expect(window).toContain('onReadClick()')
    expect(window).toContain('smart-feed-read-cta')
    expect(window).not.toMatch(/href=\{[^}]*haber/)
  })
})

describe('Swipe Discovery V7', () => {
  it('copy Haberi Aç, RIGHT travel, pointer-events none, capability gated', () => {
    ...
    expect(client).toContain("onSwipeAffordanceActivate")
