...
function baseSelect() {
  return {
    ...
    publisherSlug: sql<string | null>`coalesce(${publishers.slug}, ${newsSources.id})`,
    publisherName: sql<string | null>`coalesce(${publishers.displayName}, ${newsSources.name}, ${news.source})`,
    publisherLogoUrl: publishers.logoUrl,
    publisherVerified: sql<boolean>`${publishers.verificationStatus} = 'VERIFIED' OR ${newsSources.qualityTier} = 'TIER_A'`,
    headline: news.title,
