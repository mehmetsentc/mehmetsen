...
describe('P18 Reader dark visual + presentation override', () => {
  ...
  it('Reader + Feed source contracts for dark chrome and interactive progress', () => {
    ...
    expect(reader).toContain('selectReaderHeroCandidate')
    expect(reader).toContain('applyHeroRuntimeEvent')
    expect(reader).toContain('readerHeroShouldBeUnoptimized')
  ...
  it('page-turn transforms differ at 25/50/75%', () => {
    const feedAt = (p: number) => p * 28
    const readerAt = (p: number) => (p - 1) * 100
    expect(feedAt(0.25)).toBe(7)
    expect(readerAt(0.25)).toBe(-75)
    expect(feedAt(0.5)).toBe(14)
    expect(readerAt(0.5)).toBe(-50)
    expect(feedAt(0.75)).toBe(21)
