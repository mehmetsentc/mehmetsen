...
describe('P18.4H local feed geo gate — user geography (no proximity)', () => {
  ...
  it('city tenant Feed V2 locks corpus to Host city (Çanakkale ≠ Antalya ≠ national)', () => {
    const api = readFileSync(join(process.cwd(), 'src/app/api/feed/v2/route.ts'), 'utf8')
    const page = readFileSync(join(process.cwd(), 'src/app/(main)/feed-v2/page.tsx'), 'utf8')
    expect(service).toContain('function isCityLockedFeed')
    expect(service).toContain('lockCity?: boolean')
    expect(service).toContain('isCityLockedFeed(ctx)')
    expect(pipeline).toContain('opts.lockCity && opts.citySlug')
    expect(pipeline).toContain('input.mode !== \'local\' && !input.lockCity')
    expect(api).toContain('getCitySlugFromHeaders')
    expect(api).toContain('lockCity')
    expect(page).toContain('lockCity: Boolean(citySlug)')
    expect(client).toContain("params.set('lockCity', '1')")
    expect(client).toContain('lockCity: lockCitySlug')
    expect(candidate).toContain('const categoryIds = resolveOptsCategoryIds(opts)')
  })
})
