...
describe('surfaces reuse magazine language except Akış and desktop', () => {
  ...
  it('desktop category page uses the portal hero, not magazine stories', () => {
    ...
    expect(desktop).toContain('dcp-stage')
    expect(desktop).toContain('lead={lead}')
    expect(read('src/components/home/desktop/DesktopCategoryHero.tsx')).toContain('newsItemDetailHref')
  ...
  it('national /feed keeps magazine HomeFeed on mobile and newspaper on desktop', () => {
    ...
    expect(desktop).toContain('takePortalRail')
    expect(desktop).toContain('HOME_CATEGORY_PORTAL_FETCH')
    expect(desktop).toContain("'magazin'")
    expect(desktop).toContain('isAsayisItem')
    expect(desktop).toContain('thirdPageItems')
    ...
    expect(portal).toContain('desktop-portal-band')
    expect(portal).toContain('desktop-portal-scroll')
    expect(portal).toContain('desktop-portal-rail-x')
    expect(portal).toContain('DesktopPortalPulse')
    expect(portal).toContain('Gündem')
    expect(portal).toContain('3. Sayfa')
    expect(portal).toContain('yerel-haber')
    ...
    expect(header).toContain('DesktopWebHeader')
    ...
    expect(ssrRails).toContain("'asayis'")
    expect(types).toContain('HOME_CATEGORY_PORTAL_FETCH = 10')
  })
