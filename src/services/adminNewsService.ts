...
function adminNewsDocToPost(id: string, data: NewsDocument): Post {
  ...
  return {
    ...
    socialHeadline: (data as { socialHeadline?: string }).socialHeadline?.trim() || '',
    socialStorySummary: (data as { socialStorySummary?: string }).socialStorySummary?.trim() || '',
    socialCaption: (data as { socialCaption?: string }).socialCaption?.trim() || '',
    pushTitle: (data as { pushTitle?: string }).pushTitle?.trim() || '',
    pushText: (data as { pushText?: string }).pushText?.trim() || '',
    imageAlt: (data as { imageAlt?: string }).imageAlt?.trim() || '',
    imageFilename: (data as { imageFilename?: string }).imageFilename?.trim() || '',
    videoAlt: (data as { videoAlt?: string }).videoAlt?.trim() || '',
    videoFilename: (data as { videoFilename?: string }).videoFilename?.trim() || '',
    readingTimeMinutes: (data as { readingTimeMinutes?: number }).readingTimeMinutes,
    ...
    additionalImages: Array.isArray(data.additionalImages)
      ? data.additionalImages
          ...
          .map((img) => ({
            ...
            alt: (img as { alt?: string }).alt?.trim() ?? '',
            filename: (img as { filename?: string }).filename?.trim() ?? '',
          }))
      : [],
